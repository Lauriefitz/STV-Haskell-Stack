...
-- remove id and names
dropnames :: [[String]] -> [[String]]
dropnames (x:xs) = [drop 2 x | x <- xs]

cleanVotes :: [[String]]
cleanVotes = dropnames (rmEmptyStrings votes)
...
-- remove asterix and vote number
...
--Mapping names onto sorted list of numbers
finalVotes :: [[String]]
finalVotes =  map rmVoteTally (map rmdups extractVotes)
